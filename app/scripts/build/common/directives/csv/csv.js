(function() {
  ...
  return module.classy.controller({
    ...
    init: function() {
      ...
      if (this.utils.navigator.isIE9orLess()) {
        return Downloadify.create('downloadify', {
          filename: this.$.filename_,
          data: (function(_this) {
            return function() {
              var column, columns, data, i, j, k, len, len1, len2, month, ref, str;
              ...
              for (j = 0, len1 = ref.length; j < len1; j++) {
                month = ref[j];
                str = (_this.date(month.dates[0], 'MMMM')) + ";" + (_this.date(month.dates[0], 'yyyy'));
                for (k = 0, len2 = columns.length; k < len2; k++) {
                  column = columns[k];
                  str += ";" + (_this.output(month[column.field]));
                }
                data += "\n" + str;
              }
              return data;
    ...
    _columns: function() {
      return this.constants.columns.reports[this.$.name];
    },
    exportCsv: function() {
      var from, to;
      ...
      return this.modals.confirm.open({
        ...
        return function() {
          ...
          if (_this.utils.navigator.isIE9orLess()) {
