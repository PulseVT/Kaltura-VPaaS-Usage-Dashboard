  ...
  return module.classy.controller({
    ...
    _buildGraphData: function() {
      ...
      for (i = 0, len = ref.length; i < len; i++) {
        month = ref[i];
        data.push([index, month[this.$.valueField]]);
        xaxisTicks.push([index, month[this.$.labelField]]);
        if (maxDataValue < month[this.$.valueField]) {
          maxDataValue = month[this.$.valueField];
        }
        index++;
      }
